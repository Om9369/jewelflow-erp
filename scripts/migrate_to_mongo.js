...
if (!MONGO_URI) {
  ...
  console.log('Usage: node scripts/migrate_to_mongo.js "mongodb+srv://username:password@cluster.mongodb.net/jewelflow"');
  process.exit(1);
}
...
async function migrate() {
  ...
  if (employees.length > 0) {
    await Employee.insertMany(employees.map(e => ({
      name: e.name,
      email: e.email || '',
      phone: e.phone,
  ...
  // 5. Migrate Sales Invoices & Items
  ...
  for (const inv of invoices) {
    ...
    await SalesInvoice.create({
      ...
      items: invItems.map(it => ({
        ...
        pieces: it.pieces || 1,
        created_at: it.created_at || new Date()
      })),
  ...
  if (oldGold.length > 0) {
    await OldGoldTransaction.insertMany(oldGold.map(og => ({
      ...
      notes: og.notes || '',
      created_at: og.created_at || new Date()
    })));
  }
